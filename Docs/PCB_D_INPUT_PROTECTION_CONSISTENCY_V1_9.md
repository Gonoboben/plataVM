...
| Input capacitor class | 35 В minimum | ABOVE TVS CLAMP CLASS |
| Preferred transient peak | ≤32 В | CONSISTENT |
| Engineering maximum acceptance | ≤36 В | MEASUREMENT GATE |
...
Текущий prototype input capacitor class:
...
```text
SMCJ18A datasheet clamp ≈29,2 В
25-В capacitor rating < clamp class
```

Следовательно:

```text
25-В input capacitor class = REJECTED FOR PROTOTYPE
35-В input capacitor class = PRELIMINARY ACCEPTED
```

Если measured peak приближается к 35 В либо repetitive stress/derating недостаточны, требуется 50-В class или redesign clamp.
...
Cd 330 мкФ / 35 В: target height ≤12 мм, exact fit OPEN
...
35-В damping/input capacitors могут увеличить площадь по сравнению с 25-В class; окончательный fit проверяется после exact candidate selection.
...
exact 35-В Cd/input capacitor candidates
exact Rd pulse network
...
35-В capacitor correction: PASS
