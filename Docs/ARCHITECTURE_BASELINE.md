...
```text
V1.9 OWNER-INPUT-REVIEWED BASELINE
Architecture A/B preserved; guarded SERVICE_OVERRIDE, packaging boundary and physical interface count resolved
```

## 1. Назначение

Документ фиксирует действующую архитектуру питания, управления, резервирования, аварийного отключения, многоплатного разбиения, общего power budget, сервисной диагностики и механического envelope электронной сборки.
...
| SERVICE_OVERRIDE | guarded service function | ACCEPTED |
...
2. PCB-B — critical power, MCU, supervisor, внешний RS-485, внутренняя CAN-FD, service logic и аппаратная аварийная логика;
...
Предварительные signal classes:

```text
A↔B direct control/diagnostics: 32 positions target
B↔C signal class: 8 positions
B↔D signal class: 8 positions
B↔E signal class: 8 positions
PCB-B service/debug: 10 positions
```

A↔B допускается разделить на 16-position CTRL и 16-position DIAG после packaging review.

Физическая topology CAN-FD и termination определяются после окончательного node order.
...
- требует сервисной авторизации и двойного подтверждения;
- действует только для одного выбранного внешнего некритического output;
- имеет одноразовое окно 60 с;
- активный output автоматически отключается не позднее 60 с;
- не изменяет short limit ≤1 с;
- predicted current выше short limit блокируется;
- полностью журналируется;
- отменяется при reset, communication loss, смене режима, battery state change, fault, SAFE/HARD_OFF или EXT_KILL.

Override не применяется к K_BAT, MAIN_SW, BALANCE_SW, PACK_BUS_DISCHARGE, critical rails, REMOTE_OFF, safety commands или DECK_BALANCE.

Если overridden output удерживает ток выше continuous limit более 1 с, отключается только этот output. Остальные работающие нагрузки обычным software manager не сбрасываются.
...
Винты, стойки и крышечная механика — owner-controlled scope. PCB должны иметь mounting-hole zones и keepout.

Отдельный обязательный carrier/tray не применяется.

## 17. Preliminary packaging candidate

Принят для инженерной проработки, но не заморожен:
...
Предварительные area budgets:

| Плата | Target outline | Preliminary maximum | Component height |
|---|---:|---:|---:|
| PCB-A | 94 × 110 мм | 100 × 120 мм | ≤23 мм |
| PCB-B | 94 × 180 мм | 100 × 220 мм | ≤16 мм |
| PCB-C | 94 × 130 мм | 100 × 145 мм | ≤23 мм |
| PCB-D | 94 × 125 мм | 100 × 140 мм | ≤23 мм |
| PCB-E | 94 × 110 мм | 100 × 130 мм | ≤23 мм |
...
Размеры не являются final PCB outlines.

## 18. Mounting-hole policy

До выбора владельцем винтов и стоек:
...
Для каждой платы предусматриваются не менее четырёх mounting-hole zones, tool access и дополнительные опоры тяжёлых компонентов.

## 19. Вибрационная фиксация
...
11. thermal contact с корпусом запрещён;
12. SERVICE_OVERRIDE не отменяет аппаратные защиты;
13. part numbers выбираются только после закрытия входов, расчёта и проверки.
...
3. preliminary KiCad outlines and mounting-hole zones;
4. component-height/3D clearance audit;
5. A↔B connector split decision;
6. CAN-FD node order/termination;
7. current calculations for PCB-B/D/E branches;
8. connector class comparison and selection;
9. thermal calculations/tests without hull contact;
10. release ERC.
